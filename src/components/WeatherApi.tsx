  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (projectFireRef.current) {
        ...
        if (rect.top >= 0 && (window.innerWidth <= 768 || rect.bottom <= window.innerHeight + 200)) {
      ...
      <div className="flex flex-wrap justify-center sm:gap-20">
        <div className="z-30 mb-7 flex w-3/4 min-w-[250px] flex-col justify-center gap-4 font-almamonoLight text-2xl text-white sm:text-4xl md:w-1/4">
          ...
          <p className="max-w-full text-justify text-sm sm:text-lg">
            I built a weather API that can be used to get the current weather conditions of any city in the world. I
            used Express.js to build the API. The features of this API include: data caching, user authentication,
            request logging, swagger and postman documentation and{" "}
            <a className="cursor-pointer font-bold underline">more</a>
          </p>
          <div
            onClick={() => {
        ...
        <div className="z-30 flex w-10/12 items-center md:w-1/2">
          <img src={codeScreenshot} className="z-[29] w-full rounded-xl object-contain"></img>
        </div>
      </div>
    </div>
  );
